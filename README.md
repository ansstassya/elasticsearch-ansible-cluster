...
Роль прогнана через `ansible-lint` (профіль `production` — найвищий рівень строгості), усі зауваження виправлені:
- усі модулі через FQCN (`ansible.builtin.*`)
- виправлено приховану помилку в `notify:` — handler не спрацьовував через розбіжність у регістрі назви
- явні права доступу (`mode`) на конфігураційні файли
- `pipefail` для shell-команд з пайпами
